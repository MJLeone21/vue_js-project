...
<template>
  <section class="form-card">
    ...
    <form @submit.prevent>
      <div class="form-grid">
        <label>Plate Number<input v-model.trim="form.plateNumber" type="text" placeholder="UBK 123A"
            @input="updateParent" /></label>
        <label>Owner Name<input v-model.trim="form.ownerName" type="text" placeholder="Owner's full name"
            @input="updateParent" /></label>
        <label>Vehicle Class
          <select v-model="form.vehicleClass" @change="updateParent">
            <option>Heavy</option>
            <option>Small</option>
